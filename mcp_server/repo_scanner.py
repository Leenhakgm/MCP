...
def load_ignore_patterns(config_path: str | None = None) -> Set[str]:
    ...
    try:
        ...
        values = set(data.get("ignore_patterns", []))
        return values | DEFAULT_IGNORE_PATTERNS
    except Exception:
...
def _safe_local_root(path: str, workspace_root: Path) -> Path:
    ...
    if not str(candidate).startswith(str(workspace_root.resolve())):
        raise ValueError("Path traversal blocked: path must stay inside workspace")
...
def generate_validation_plan(secret: str, context: str, infer_service) -> Any:
    """Run LLM contextual classification for a detected secret candidate."""
    return infer_service(secret, context)


def safe_execute(secret: str, endpoint: str, execute_validation) -> ValidationExecutionResponse:
...
def validate_plan(secret: str, endpoint: str, execute_validation) -> ValidationExecutionResponse:
    return safe_execute(secret, endpoint, execute_validation)
...
def _process_detection(
    ...
    if cached:
        ...
        if (
            detection.confidence >= min_validation_confidence
            and inference.service != "unknown"
            and inference.validation_plan
        ):
            endpoint = inference.validation_plan.endpoint

            if inference.validation_plan.endpoint != "unknown":
                validation = validate_plan(
                detection.secret,
                inference.validation_plan.endpoint,
                execute_validation
                )
            else:
                validation = None
    ...
    return RepoScanResult(
        ...
        if validation
        else {"status": "SKIPPED", "reason": "No validation performed"},
    )

def _export_report(report: RepoScanResponse, output_file: str | None) -> None:
    if not output_file:
        return
...
def scan_repository(
    ...
    try:
        ...
        logger.info("repo_scan_files_discovered count=%s", len(source_files))

        # LLM-driven contextual detection (detector delegates to Gemini; no regex patterns).
        detections = detect_secrets(
            DetectionRequest(files=source_files, min_confidence=request.min_detection_confidence)
        )
        logger.info("repo_scan_llm_secrets_detected count=%s", len(detections))
                ...
                try:
                    ...
                    results.append(
                        RepoScanResult(
                            ...
                            validation_result={"status": "ERROR", "reason": str(exc)},
                        )
                    )

        valid_count = sum(1 for r in results if r.validation_result.get("status") == "VALID")
...
async def scan_repository_async(
    ...
    return await asyncio.to_thread(scan_repository, request, infer_service, execute_validation, workspace_root)