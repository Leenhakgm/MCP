...
def reconstruct_split_secrets(content: str):
    ...
    for line in lines:
        ...
        # detect start of a token with prefix
        if line.startswith(("hf_", "ghp_", "sk_", "gsk_", "AIza", "xoxb-")):

            # start a new buffer
            buffer = line
            continue

        # if we already started a token, append fragments
        if buffer and line.isalnum():

            buffer += line

            # if token becomes long enough assume complete
            if len(buffer) >= 30:
